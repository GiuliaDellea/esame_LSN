...
{ 
  ...
  //Equilibration
  //ofstream Equil; 
  //Equil.open("Equil(T=2)G.dat"); //T=0.3, 0.5, 1,2,3, 0.3 con h=0.02 invece che nullo
  for(int i=0;i<1000;i++){
    Move(metro);
    //Measure();
    //Equil  << walker[im]/(double)nspin << endl;
  }
  //Equil.close();
  //Ripeto simulazione con T=0.5, 0.6,...fino a T=2 sia con Metr che con Gibbs
...
void Move(int metro)
{
  ...
  //Select randomly a particle (for C++ syntax, 0 <= o <= nspin-1)
    ...
    else //Gibbs sampling
    {
      ...
      if(rnd.Rannyu()<1./(1+exp(2*beta*energy_up)))
	s[o]=1;
      else
	s[o]=-1;
    }
...
void Averages(int iblk) //Print results for current block
{
   ...
   //const int wd=12;
    ...
    Heat.open("output.heat.G",ios::app);
