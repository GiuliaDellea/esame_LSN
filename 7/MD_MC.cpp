...
{ 
  ...
  int nconf = 1;
  /*ofstream Equil; 
    Equil.open("Equil_s5.dat");*/
  for(int i=0; i<1000; i++)  //Equilibration
    Move();
  /*for(int i=0; i<500000; i++){  //Produzione 500.000 dati
     Move();
     Measure();
     Equil <<  walker[iv]/(double)npart + vtail << endl;
     }
  Equil.close();
...
//Read input informations
  ...
  //Tail corrections for potential energy and preassure
...
//Print initial values for measured properties
  ...
  cout << "Initial pressure (with tail corrections) = " << rho * walker[it] + (walker[iw] + (double)npart * ptail)/vol << endl;
...
void Averages(int iblk) //Print results for current block
{
  ...
  ofstream Epot, Ekin, Etot, Temp, Pres, Gr;
   //const int wd=12;
    ...
    stima_pres = rho * walker[it] + (blk_av[iw]/blk_norm + (double)npart * ptail)/vol; //Pressure
...
//g(r)
    ...
    for(int i=0;i<nbins;i++){
      r=i*bin_size;
      DeltaV=(4.*pi/3.)*(pow(r+bin_size,3)-pow(r,3));
      g2[i]=double(g2[i]/(double)blk_norm)/(rho*m_part*DeltaV);
    }

    for(int i=0;i<nbins;i++){ 
      Gr << iblk << " " << g2[i] << " " << err_g << endl;
    }
