...
{ 
  ...
  int nconf = 1;
  /*ofstream Equil; //Equilibration
  Equil.open("Equil_gas.dat");
  for(int i=0; i<2000; i++){
     Move();
     Measure();
     Equil << walker[it] << endl;
  }
  Equil.close();*/
...
void Input(void)
{
  ...
  cout << "MD(NVE) simulation       " << endl << endl;
...
//Print initial values for measured properties
  ...
  cout << "Initial pressure      = " << rho * walker[it] + walker[iw]/vol << endl;
  //cout << walker[iw]/vol << endl;
  //cout << rho * temp << endl;
  return;
}
...
void Averages(int iblk) //Print results for current block
{
  ...
  ofstream Epot, Ekin, Etot, Temp, Pres;
   //const int wd=12;
    ...
    stima_pres = rho * walker[it] + blk_av[iw]/blk_norm/vol; //Pressure
